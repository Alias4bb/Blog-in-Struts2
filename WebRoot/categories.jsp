...
<% 
 ...
 %>
    <title> 分类:<%=type %> | Alias's Blog </title>
</head>
...
ResultSet rs;
 ...
 String sql="select * from blog_contents where c_type='"+type+"'";
  rs=db.doSelect(sql);
 while(rs.next()){%>
    ...
    <header class="post-header">
      ...
      <h1 class="post-title">
            ...
            <a class="post-title-link" href="Posts?id=<%=rs.getString("c_id") %>" itemprop="url">
              <span itemprop="name"><%=rs.getString("c_title") %></span>
            </a>
        
      </h1>
<%     
  String date=rs.getString("c_create");
