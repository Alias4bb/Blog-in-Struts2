...
<% ResultSet rs,ts,ss;
  ...
  String sql="select * from blog_contents where c_id='"+id+"'";
 Db db=new Db();
 rs=db.doSelect(sql);
 String sql3="select t_id from blog_relationships where  c_id='"+id+"'";

 ts=db.doSelect(sql3);
 if(rs.next()){
 %>
    <title> <%=rs.getString("c_title") %> | Alias's Blog </title>
</head>
...
<nav class="site-nav">
  ...
  <article class="post post-type-normal" itemscope itemtype="http://schema.org/Article">
    <header class="post-header">
     ...
     <h1 class="post-title" itemprop="name headline"><%=rs.getString("c_title") %>
           </h1>
      ...
      <div class="post-meta">
         ...
         发表于
          ...
          <span class="post-category" >
            &nbsp; | &nbsp; 分类于
              ...
              <span itemprop="about" itemscope itemtype="https://schema.org/Thing"><a href="Categories?type=<%=rs.getString("c_type") %>" itemprop="url" rel="index"><span itemprop="name"><%=rs.getString("c_type") %></span></a></span>
        ...
        <span class="posts-comments-count"> &nbsp; | &nbsp;
        <span class="ds-thread-count" data-thread-key="<%=rs.getString("c_id") %>" data-count-type="comments"></span>
        </span>
...
<div class="post-body">
 ...
 <span itemprop="articleBody"><%=rs.getString("c_content") %></span> </div>
    
  <footer class="post-footer">
      
        <div class="post-tags">
          <% while(ts.next()){ 
           String t=ts.getString("t_id");
            String sql5="select t_name from blog_tags where t_id='"+t+"'";
           ss=db.doSelect(sql5);
           while(ss.next()){
          %>
            <a href="Tags?name=<%=ss.getString("t_name") %>" rel="tag">#<%=ss.getString("t_name") %></a>
          <%}} %>
        </div>
    ...
    <div class="post-spread">
              ...
              <div class="comments" id="comments">
                <div class="ds-thread" data-thread-key="<%=rs.getString("c_id") %>" data-title="<%=rs.getString("c_title") %>" data-url="http://127.0.0.1:8080/MyHomePage/Posts?id=<%=rs.getString("c_id") %>"></div>
<!-- 多说评论框 end -->
