...
<% 
 ...
 %>
    <title> 标签:<%=name %> | Alias's Blog </title>
</head>
...
ResultSet rs,ss;
      String t_id="";
 String sql="select t_id from blog_tags where t_name='"+name+"'";
  rs=db.doSelect(sql);
 if(rs.next()){
  t_id=rs.getString("t_id");
 }
 String sql3="select c_id from blog_relationships where t_id='"+t_id+"'";
 rs=db.doSelect(sql3);
 while(rs.next()){
 String sql5="select * from blog_contents where c_id='"+rs.getString("c_id")+"'";
 ss=db.doSelect(sql5);
 if(ss.next()){
 %>
  <article class="post post-type-normal"itemscope itemtype="http://schema.org/Article">
    <header class="post-header">
      ...
      <h1 class="post-title">
            ...
            <a class="post-title-link" href="Posts?id=<%=ss.getString("c_id") %>" itemprop="url">
              <span itemprop="name"><%=ss.getString("c_title") %></span>
            </a>
        
      </h1>
<%     
  String date=ss.getString("c_create");
  String date1=date.substring(5,10);
  %>
      <div class="post-meta">
        <time class="post-time"  itemprop="dateCreated" ><%=date1 %></time>
      </div>
...
<div class="powered-by">
  由 <a class="theme-link" >Struts2</a> 强力驱动
</div>
